        ...
        <div ref={headerRef} className={`reveal ${headerVisible ? 'visible' : ''}`}>
          <div className="text-center mb-16">
            ...
            <h2 className="font-heading text-3xl md:text-5xl text-foreground font-bold">
              Our Booths — Find the Perfect Match for Your Celebration
            </h2>
          </div>
        </div>
        ...
        <div ref={ref}>
          <div className="hidden md:grid md:grid-cols-3 gap-6 mb-6">
            {services.slice(0, 3).map((service, i) => (
              <div key={service.href} className={`reveal reveal-delay-${i + 1} card-hover-lift ${isVisible ? 'visible' : ''}`}>
                <ServiceCard service={service} fixedHeight />
              </div>
            ))}
          </div>
          <div className="hidden md:grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {services.slice(3).map((service, i) => (
              <div key={service.href} className={`reveal reveal-delay-${i + 4} card-hover-lift ${isVisible ? 'visible' : ''}`}>
...
const ServiceCard = ({ service, fixedHeight }: ServiceCardProps) => (
  <Link to={service.href} className={`group block relative overflow-hidden rounded-2xl border-2 border-primary/40 transition-all duration-500 ${fixedHeight ? 'h-[500px]' : 'aspect-[4/5]'}`} style={{ boxShadow: 'inset 0 1px 0 0 hsl(45 65% 54% / 0.5), inset 0 -2px 4px 0 hsl(38 70% 30% / 0.4), 0 4px 16px hsl(42 72% 42% / 0.2), 0 8px 32px hsl(42 72% 42% / 0.15), 0 1px 3px hsl(0 0% 0% / 0.4)' }}>
    {service.isPremium && (
      <div className="absolute top-3 right-3 z-10">
        <span className="bg-primary text-primary-foreground text-[10px] font-medium px-2.5 py-1 rounded-full tracking-[0.07em] uppercase">
          Premium
        </span>
      </div>
    )}
